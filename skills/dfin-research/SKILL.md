...
name: dfin-research
description: >-
  Research stocks and public companies with dfin.pro's source-grounded financial data. Use when the user asks to analyze a company, compare peers, pull financial statements and ratios, price data, find evidence in SEC filings, transcripts, or dfin.pro reports.
---

# dfin.pro Research

Answer financial questions with source-grounded data from the dfin.pro MCP: securities lookup, company SEC filings, earnings-call transcripts, dfin.pro stock analysis reports, latest transcript/report discovery, explicit full-document retrieval, financial statements, ratios, stock context, and current and historical prices. Use a dfin-first approach. Filings, financial statements, and ratios cover US companies only. International coverage is selective — transcripts and prices are commonly available — so let `get_stock_context` tell you what exists for a given name instead of assuming, and move on if a call comes back empty. Fall back to public searches only for what dfin does not cover.

## Core rules

- **Read the agent guide before non-trivial work.** For anything multi-step — peer comparisons, evidence gathering across sources, or work spanning several tool areas — call `agent_help(topic="agent_guide")` (alternate direct link: https://www.dfin.pro/docs/mcp/agent-guide.md) before you start. It is the routing and parameter authority for source search, latest browsing, prices, financials/ratios, and stock context. Skip it for a single obvious lookup such as a current price or one ticker resolution: each tool's own schema already carries its contract, and the guide's value is sequencing, not tool discovery. Call it mid-task if routing turns out to be unclear, or if a call rejects a parameter (API drift).
...
- **Get stock context for company research.** When the task is genuine research on a company — analysis, diligence, peer work, or evidence gathering — call `get_stock_context` immediately after resolving the ticker and before statements, ratios, filings, transcripts, reports, or other company-specific evidence gathering. Narrow it with `fields` whenever you already know which sections the task needs — it returns every section by default, and a coverage check before an evidence search or a returns question rarely needs all of them. Skip the call entirely for a mechanical one-hop lookup that a single tool already answers, such as a current price or a price history for a chart.
...
Prefer targeted searches for filings, transcripts, and reports. Company-specific source research is research, so resolve the ticker and get stock context first, per the core rules.
...
- **Follow-up inside documents you already found:** Once a search, latest-document, or report-details call has returned `doc_uuid` values, use `search_in_documents(doc_uuids=[...], queries=[...])` to search only those documents rather than re-running a broad search. It takes up to 20 `doc_uuid` values and up to 12 queries, and it is the cheapest way to pin down detail in a filing, transcript, or report already in hand. Reach for `get_document_content` only when you need surrounding full text rather than an answer.
...
Read the agent guide first for the multi-step flows below, per the core rule; the one-hop lookups don't need it. You don't need to re-run search_securities or get_stock_context for a company you already resolved or fetched context for earlier.
...
- **Price or chart request:** `search_securities` -> `get_price`. Add `get_stock_context` only if the request widens into research beyond the price itself.
- **Follow-up on a document already found:** `search_in_documents` with the `doc_uuids` from the earlier result -> `get_document_content(doc_uuid, chunk_num)` only if you still need the surrounding text.
